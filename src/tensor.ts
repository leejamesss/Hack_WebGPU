...
export class Tensor extends TensorBase {
        ...
        if (arrayOrSpec === null) {
            ...
            if (jdata.data instanceof Array) {
                ...
                if (jdata.shape === undefined && jdata.strides === undefined) {
                    throw new Error(
                        "Cannot create tensor from storage without also specifying the shape and strides."
                    );
                }
    ...
    runKernelInplace(
        ...
        if (this.requiresGrad) {
            throw new Error(
                `A tensor that requires a gradient cannot be used in an in-place operation`
            );
        }
        const d = this.device;
    ...
    async broadcastTo(targetShape: Shape): Promise<Tensor> {
        ...
        if (RankDelta > 0) {
            const ones = Array(RankDelta).fill(1);
            expandedShape = [...ones, ...expandedShape];
            for (let i = 0; i < RankDelta; i++) {
                // ensure the match between shape and array
                newArray = [newArray.slice()];
            }
        }

        // Now dimension matches, start boardcasting
        if (expandedShape[0] != targetShape[0]) {
            newArray = Array(targetShape[0])
                .fill(-1)
                .map(() => newArray[0].slice());
            expandedShape[0] = targetShape[0];
        }
        for (let level = 1; level < targetRank; level++) {
            if (expandedShape[level] != targetShape[level]) {
                expandedShape[level] = targetShape[level];
                let tmp = expandedShape.slice(0, level).map((x) => range(x));
                for (let indexArray of product(...tmp)) {
                    let preLevelElement = newArray;
                    for (const index of indexArray) {
                        preLevelElement = preLevelElement[index];
                    }
                    for (let j = 1; j < targetShape[level]; j++) {
                        preLevelElement[j] = preLevelElement[0];
                    }
                }
            }
